        ...
        child._children.forEach(elem => {
            ...
            if (key === 'res') {
                for (const attr in elem.attrib) {
                    item.res[attr] = elem.attrib[attr]
                }
                const [audioType] = (item.res.protocolInfo || '').match(/audio\/\w+/) || ['']
                item.allRes[audioType] = item.res.url = elem.text
...
function buildMetadata(metadata) {
    ...
    if (metadata.url && metadata.protocolInfo) {
        const res = elementTree.SubElement(item, 'res')
        res.set('protocolInfo', metadata.protocolInfo)
        res.text = metadata.url
    }

    if (metadata.subtitlesUrl) {
        const captionInfo = elementTree.SubElement(item, 'sec:CaptionInfo')
        captionInfo.set('sec:type', 'srt')
        captionInfo.text = metadata.subtitlesUrl

        const captionInfoEx = elementTree.SubElement(item, 'sec:CaptionInfoEx')
...
server.listen(8090, () => console.log(`server started at port ${8090}`))
...
app.use(koaRoute.post('/upnp-content-directory/:method', async (ctx, method) => {
    ...
    ctx.response.body = parseBrowseResult(resp.Result)
}))
...
async function playNext(url, ins, playingTrack, playingQueue) {
    await upnpCall(url, 'SetAVTransportURI', {
        ...
        CurrentURIMetaData: buildMetadata(playingTrack.res),
    })
...
app.use(koaRoute.post('/upnp-avtransport/:method', async (ctx, method) => {
    ...
    if (method === 'SetAVTransportURI') {
        inputs.CurrentURIMetaData = buildMetadata(inputs.res)
    }
    ctx.response.body = await upnpCall(url, method, inputs)
...
function getServiceData(service) {
    ...
    return { location: SCPDURL, st: serviceType, server: device.friendlyName }
}
