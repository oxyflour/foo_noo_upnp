const http = require('http'),
    ...
    jimp = require('jimp'),
    lame = require('lame'),
    gateway = require('default-gateway'),
    ...
    { devOpts, contentDirectoryDescription, avTransportDescription, renderingControlDescription } = require('../common/upnp')
...
function createBrowseResult(array, baseUrl) {
    ...
    array.forEach(item => {
        ...
        if (item.type === 'container') {
            ...
            if (ext === 'flac') {
                addSub(elem, 'res', `${baseUrl}/decode/${path}/subsong${item.subsong}.flac`, {
                    duration,
            ...
            addSub(elem, 'res', `${baseUrl}/decode/${path}/subsong${item.subsong}.mp3`, {
                duration,
...
const { interface } = await gateway.v4(),
    infs = os.networkInterfaces(),
    addrs = infs[interface] || Object.values(infs).pop(),
    hostname = (addrs || [ ]).filter(addr => addr.family === 'IPv4' && !addr.internal)
        .map(addr => addr.address).pop() || os.hostname()
